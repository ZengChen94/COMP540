...
class RegularizedLinearRegressor_Multi:
    ...
    def predict(self, X):
        ...
        y_pred = np.dot(np.transpose(X), self.theta)
...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        if len(X) != 0:
            ...
            J = (np.dot(np.transpose(delta), delta) + reg * np.dot(theta[1:], np.transpose(theta[1:])))/len(X)/2.0
    ...
    def grad_loss(self,*args):                                                                          
        ...
        delta = np.dot(X, theta) - y
        if len(X) != 0:
            grad[0] = np.transpose(X[:, 0]).dot(delta) / float(len(X))#don't take mean() here!!!
            grad[1:] = (np.transpose(X[:, 1:]).dot(delta) + reg * theta[1:]) / float(len(X))
